...
const ProductCart = ({productInCart, img, nameProduct, price, store, quantity, unity, 
  ...
  return (
    <div className="box-pro-cart d-flex flex-wrap p-2 mb-2">
      ...
      <div className="info-pro-cart col p-0 pl-2 d-flex flex-column justify-content-around">
        ...
        <p className="m-0">{price.toLocaleString("en", {style: "currency", currency: "USD"})}</p>
        <p className="m-0">{store}</p>
      </div>
      <div className="cantidad-pro-cart text-center align-self-center col pt-3 p-0">
        <div className="d-flex justify-content-center">
          <button className="btn-quantity" onClick={() => handleDecrement(productInCart)}>-</button>
          <input readOnly type="number" min="1" value={quantity}/>
          <button className="btn-quantity" onClick={() => handleIncrement(productInCart)}>+</button>
        </div>
